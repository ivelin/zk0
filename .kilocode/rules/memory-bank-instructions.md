...
**RESPONSES at the start of a NEW TASK MUST START WITH:**

```
[Memory Bank: Active]
Project: [current project name]
Status: [brief status summary]
Last Updated: [memory bank date]
```

This appears ONLY at the beginning of responses to new tasks. For continuations within an ongoing task, omit the summary to avoid repetition.

## Pre-Response Checklist (MANDATORY)

Before generating ANY response at the start of a NEW TASK:

- [ ] **Memory Bank Status**: Include `[Memory Bank: Active]` at response start
- [ ] **Project Context**: Reference current project and status

**FAILURE TO COMPLETE THIS CHECKLIST**: Response must be regenerated with proper format. For ongoing task continuations, no summary needed.
...
In the beginning of a NEW TASK I MUST read ALL memory bank files - this is not optional.

The memory bank files are located in `.kilocode/rules/memory-bank` folder. If the folder doesn't exist or is empty, I will warn user about potential issues with the memory bank. I will include `[Memory Bank: Active]` at the beginning of my response ONLY for new tasks if I successfully read the memory bank files, or `[Memory Bank: Missing]` if the folder doesn't exist or is empty. If memory bank is missing, I will warn the user about potential issues and suggest initialization. I should briefly summarize my understanding of the project to confirm alignment with the user's expectations, like:
...
1. **Automatic Checks**: Responses at the start of a new task must include rule compliance verification
2. **No Exceptions**: Rules apply to responses at the start of new tasks
3. **Self-Monitoring**: Include rule compliance status in response metadata for new tasks
4. **Correction Required**: Any violation requires response regeneration

**ENFORCEMENT MECHANISM**:
- Memory Bank Active: Must appear in first line of responses to new tasks only
- File Edits: VSCode automatically provides diff review windows for all apply_diff operations
- Rule Violations: Must be corrected immediately
