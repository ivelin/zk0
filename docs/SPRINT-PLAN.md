...
- **Node Operator Onboarding**: Onboarding via GitHub issues (new template) and Discord; detailed docs for private dataset integration.
...
**Versioning**: Bump from 0.3.16 to 0.4.1 (minor for production support and testing improvements). Use semantic versioning; tag Docker images accordingly.
...
- **Docker Compose**: Single `docker compose up` for quickstarts; separate `compose.yml` for server/SuperLink and clients/SuperNodes. Ports: 9091 (ServerAppIO API), 9092 (Fleet API), 9093 (Control API). No app-level connection security (e.g., invite keys) needed in client/server apps; communication security handled externally via VPN-like P2P tools (Tailscale/WebRTC) at network level, allowing Flower to run in insecure mode.
...
- **Other Differentiations**:
  ...
  - **Auth**: Simulation: None. Prod: Clients and server run in insecure mode with no app-level security (e.g., no invite keys or auth tokens); all secure communication handled externally via VPN-like P2P tools (Tailscale/WebRTC) at network level.
...
- **Docker Setup**:
  - **Custom Image**: `Dockerfile.zk0` extends `huggingface/lerobot-gpu:latest` (GPU-only for production setups); installs zk0 deps, copies src. Build: `docker build -t ghcr.io/ivelin/zk0:v0.4.0 -f Dockerfile.zk0 .`. Push: `docker push ghcr.io/ivelin/zk0:v0.4.0`.
  - **Compose Files**:
...
- **Dataset Handling**:
  - Prod Clients: Startup via run_config `dataset.repo_id="user/repo"` for HF or `dataset.root="/path/to/local"` for local (matches LeRobot CLI --dataset.repo_id and --dataset.root); load via LeRobot (private HF needs HF_TOKEN env).
...
- **Onboarding**:
  - **Node Operators Doc**: `docs/NODE-OPERATORS.md` – Steps: Apply via GitHub issue (template: setup details, dataset uniqueness, Discord handle); install zk0bot (`curl -fsSL https://get.zk0.bot | bash`); run `zk0 client start --dataset yourrepo/yourdsname`.
...
- [x] Bump version to 0.4.1 in pyproject.toml
...
- [ ] Push to GHCR (setup GitHub actions for auto-build/push on tag only if build succeeds and tests pass; GPU-only, no non-GPU platforms)
- [x] Create docker-compose.server.yml (SuperLink + ServerApp, ports 9091-9093, zk0 entrypoint)
- [x] Create docker-compose.client.yml (SuperNode + ClientApp, dataset volume, env for dataset URI)
...
- [x] Runtime Mode Implementation
- [x] Rename train.sh to train-fl-simulation.sh and update for simulation-only use
- [x] Integrate server/client startup into zk0bot CLI (subcommands like zk0bot server start, zk0bot client start --dataset URI; wraps Docker Compose internally, no separate scripts needed)
- [x] Refactor server_app.py: Mode guards for simulation (Ray) vs prod (external connects); <500 LOC, split if needed (e.g., server_prod.py)
- [x] Refactor client_app.py: Prod mode loads dataset from run_config (dataset_repo_id for HF, dataset_root for local, matching LeRobot CLI); append UUID metrics; <500 LOC
- [x] Update configs/datasets.py: Prod client ignores partitions; server expands eval list (add 2 placeholder SO-101 datasets)
- [x] Update wandb_utils.py: Use client_id prefix for metrics (anonymized)
- [x] Node Operator CLI Tool (zk0bot)
- [x] Research and design zk0bot CLI (inspired by RocketPool rocketpool CLI and Opencode installer: bash one-step install via curl -fsSL https://get.zk0.bot | bash, commands like zk0 server start/stop/log, zk0 client start --dataset repo/name, zk0 status, zk0 config)
- [x] Implement zk0bot as bash-based CLI (single binary or script bundle, hosted on GitHub releases; installer downloads and sets up PATH; integrates Docker/Compose for server/client control)
- [x] Create installer script (get.zk0.bot -> curl -fsSL https://get.zk0.bot | bash, detects OS, downloads binary, adds to PATH, verifies Docker/Compose)
- [x] Add zk0bot commands: server start/stop/log/status, client start/stop/log --dataset URI, config (env vars, dataset setup)
- [x] Add zk0bot installation guide to docs/NODE-OPERATORS.md (one-step curl install, quick start examples)
- [ ] Test zk0bot: Verify installer on Ubuntu/Mac/Windows (WSL), commands integrate with startup scripts (e.g., zk0 server start)
- [x] Documentation & Onboarding
- [x] Create docs/NODE-OPERATORS.md: Steps for apply (GitHub issue), client setup with zk0bot, server overview, Discord link
- [x] Create .github/ISSUE_TEMPLATE/node-operator-application.md: Form fields (hardware, dataset info, etc.)
- [x] Update README.md: Add production section with zk0bot examples, startup scripts
- [x] Update docs/ARCHITECTURE.md: Add prod mode diagram, differences table, CLI integration
- [x] Update docs/DEVELOPMENT.md: Docker testing guidelines, <500 LOC rule
- [x] Code Reorganization
- [x] Audit src/: Split large files (e.g., server_app.py → server_core.py + server_prod.py); ensure <500 LOC new files
- [x] Move utils to src/core/ for modularity
- [ ] Update tests: Add prod mode integration tests (mock external connects); run in Docker
- [x] Enforce <500 LOC rule: Add to memory-bank/tech.md ("New source files must be <500 lines for maintainability")
- [ ] Testing & Validation
- [x] Run full pytest in Docker (coverage 36.67%, above 30% target; include prod mocks) - Version bumped to 0.4.1 for testing improvements
...
- [ ] Finalization
- [ ] Update memory-bank/context.md: v0.4.1 progress, prod features
- [ ] PR: Merge feat-prod-deployment to main; tag v0.4.1
